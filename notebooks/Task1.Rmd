...
```{r}
df <- df %>%
  mutate(thangtest = ifelse(Month.of.the.year%%2==0, "chan", "le"))
print(df)
```
...
#chart of Conversion
ggplot(conversion_rate, aes(x = factor(YearMonth), y = total_conversion, fill = `Source...Medium`)) + geom_line (stat = "identity", position = "dodge") + 
...
summary_stats <- df2(
  ...
  Variance = sapply(var, na.rm = TRUE)
)
...
```{r}
for (col in names(df2_cleaned)) {
  p <- ggplot(df2_cleaned, aes_string(x = col)) +
